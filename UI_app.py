...
def get_core_folder_path():
    """
    Get the path to the core folder, either from user input or session state.
    This allows the Streamlit app to import from a local core folder.
    """
    if 'core_path' not in st.session_state:
        st.session_state.core_path = None
    
    return st.session_state.core_path


def initialize_core_imports():
    """
    Add core folder to Python path and import necessary modules.
    Returns True if successful, False otherwise.
    """
    core_path = get_core_folder_path()
...
def import_core_modules():
    """
    Dynamically import all required modules from the core folder.
    This must be called after the core path is set.
    """
    global HardConstraintLoader
    global SoftConstraintLoader
    global run_constraints_test
    global convert_schedule_format
    # Add all other imports you need from core.*
    ...
    # Import any other core modules you use
    
    return True

def main():

    if get_core_folder_path() is None:
        # Show path input interface
        st.title("🔧 Setup Required" if lang == 'en' else "🔧 Opsætning Påkrævet")
        
        st.markdown("""
        ### Specify Core Folder Location
        Please enter the full path to your `core` folder on your local PC.
        
        **Example paths:**
        - Windows: `C:/Users/YourName/Documents/hospital-scheduling/core`
        - Mac/Linux: `/home/username/hospital-scheduling/core`
        """ if lang == 'en' else """
        ### Angiv Core Mappe Placering
        Indtast venligst den fulde sti til din `core` mappe på din lokale PC.
        
        **Eksempel stier:**
        - Windows: `C:/Users/DitNavn/Dokumenter/hospital-scheduling/core`
        - Mac/Linux: `/home/brugernavn/hospital-scheduling/core`
        """)
        
        core_path_input = st.text_input(
            "Core Folder Path" if lang == 'en' else "Core Mappe Sti",
            placeholder="C:/path/to/your/core" if lang == 'en' else "C:/sti/til/din/core",
            help="Enter the full path to the core folder containing your Python modules"
        )
        
        col1, col2 = st.columns([1, 3])
        with col1:
            if st.button("✓ Confirm" if lang == 'en' else "✓ Bekræft", type="primary"):
                if core_path_input and os.path.exists(core_path_input):
                    # Verify it's actually the core folder by checking for expected files
                    expected_files = ['data_loader.py', 'scheduling_model.py', 'constraint_loader.py']
                    if all(os.path.exists(os.path.join(core_path_input, f)) for f in expected_files):
                        st.session_state.core_path = core_path_input
                        st.success("✓ Core folder found!" if lang == 'en' else "✓ Core mappe fundet!")
                        st.rerun()
                    else:
                        st.error(
                            "⚠️ This doesn't appear to be the correct core folder. Missing expected Python files."
                            if lang == 'en' else
                            "⚠️ Dette ser ikke ud til at være den korrekte core mappe. Mangler forventede Python-filer."
                        )
                else:
                    st.error("⚠️ Path does not exist" if lang == 'en' else "⚠️ Stien eksisterer ikke")
        
        with col2:
            st.info("💡 Tip: Copy the full path from your file explorer" if lang == 'en' else 
                   "💡 Tip: Kopier den fulde sti fra din filhåndtering")
        
        return  # Don't proceed until path is set
    
    # Core path is set, initialize imports
    try:
        if not initialize_core_imports():
            st.error("Failed to initialize core imports" if lang == 'en' else "Kunne ikke initialisere core imports")
            if st.button("🔄 Reset Path" if lang == 'en' else "🔄 Nulstil Sti"):
                st.session_state.core_path = None
                st.rerun()
            return
        ...
        st.error("Failed to import core modules. Please verify the path is correct." if lang == 'en' else
                "Kunne ikke importere core moduler. Verificer venligst at stien er korrekt.")
        if st.button("🔄 Reset Path" if lang == 'en' else "🔄 Nulstil Sti"):
            st.session_state.core_path = None
            st.rerun()
        return
